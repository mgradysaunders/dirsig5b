...
void Sensor::request(std::vector<d5b::ProblemRequest> &problemRequests) {
  problemRequests.reserve(1024 * 1024);
    ...
    for (int x = 0; x < 1024; x++) {
      problemRequests.emplace_back([=, this] {
        ...
        problem.sampleRay = [=, this](d5b::Random &random) {
          double dx = (x + d5b::generateCanonical<double>(random)) / 1024.0 - 0.5;
          double dy = (y + d5b::generateCanonical<double>(random)) / 1024.0 - 0.5;
          return d5b::Ray{d5b::Vector3(0, 1000, 1000), d5b::normalize(d5b::Vector3(2 * dx, -2 * dy, -1))};
        };
...
d5b::Status Sensor::finish() {
  ...
  count += 4;
  mi::stbi::ImageU8 imageU8(mi::with_shape, 1024, 1024, 3);

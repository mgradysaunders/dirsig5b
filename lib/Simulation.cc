...
void Simulation::simulate() {
  ...
  while (true) {
    ...
    Status status{sensor->request(problemRequests)};
...
void Simulation::simulate(Problem problem) {
  ...
  for (size_t sampleIndex = 0; true; sampleIndex++) {
    ...
    SensorRay sensorRay{problem.sampleSensorRay(random)};
    Ray ray{sensorRay.ray};
    ...
    for (size_t bounceIndex = 0; problem.bounceLimit == 0 || bounceIndex < problem.bounceLimit; bounceIndex++) {
      ...
      if (estimatorIn.minBounces <= bounces && bounces <= estimatorIn.bounceLimit) {
        int sampleIndex = vert == 0 ? 16 : vert == 1 ? 8 : 1;
        for (int i = 0; i < sampleIndex; i++) {
          ...
          if (mi::isfinite(value).all() && (value > 0).any() && !rayTest(lightRay, tmpLocation))
            color += value * light->emission->emission(lightLoc.shading.localToWorld(), mi::fastNormalize(-lightRay.direction));
        }
      }
#endif

      if (vertex.scattering) {
        throughput = 1;
    ...
    if (problem.acceptContribution(sampleIndex, path, radiance) == Status::Done) break;
  }
}
